...
serveWith.get("/getFileSize", (req, res) => {
  const filename = req.query.filename;
  const filePath = path.join(__dirname, `/api/downloads/${filename}`);
  fs.stat(filePath, (err, stats) => {
    if (err) {
      console.error("Error fetching file size:", err);
      res.status(500).send("Error fetching file size");
    } else {
      const fileSize = stats ? stats.size : null; // Ensure stats object exists before accessing size property
      console.log("File size:", fileSize); // Add this line for debugging
      if (fileSize !== null && fileSize !== undefined) {
        res.send({ size: fileSize });
      } else {
        res.status(500).send("File size is null or undefined.");
      }
    }
  });
});
