...
class Uploader extends Component {
  ...
  fetchFileSize = (filename) => {
    ...
    get("/getFileSize", { params: { filename: filename } })
      .then((response) => {
        ...
        console.log("Received file size for", filename, ":", fileSize); // Add this line for debugging
        if (fileSize !== null && fileSize !== undefined) {
          ...
          console.error("Received null or undefined file size for", filename);
        }
      })
      .catch((error) => {
        console.error("Error fetching file size for", filename, ":", error);
      });
  };
  ...
  componentDidUpdate(prevProps) {
    // Check if startFetch prop has changed
    if (this.props.startFetch && this.props.uploadedFiles !== prevProps.uploadedFiles) {
      // Fetch file sizes for newly uploaded files
      this.props.uploadedFiles.forEach((file) => {
        this.fetchFileSize(file.filename);
      });
    }
  }
    ...
    return (
      <section className="main-section">
        ...
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <button className="btn-left" onClick={this.runCppCode}>
            Compress Text File
          </button>

          <button className="btn-left" onClick={this.runCppCode1}>
